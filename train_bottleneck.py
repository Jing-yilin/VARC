#!/usr/bin/env python3
"""Training script for ARCBottleneck on Apple Silicon.

Usage:
    python train_bottleneck.py --epochs 50 --batch-size 32
    python train_bottleneck.py --epochs 50 --batch-size 32 --rule-dim 16
"""
...
def train(args):
    ...
    print(f"Rule dim: {args.rule_dim}, Iterations: {args.num_iterations}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, batch in enumerate(train_loader, 1):
            ...
            if step % 10 == 0:
                ...
                done = (epoch - 1) * len(train_loader) + step
                eta = elapsed / max(done, 1) * (total_steps - done)
                pct = 100.0 * done / total_steps
                bar_len = 30
                filled = int(bar_len * done / total_steps)
                bar = "#" * filled + "-" * (bar_len - filled)
                sys.stdout.write(f"\rEpoch {epoch} [{bar}] {pct:5.1f}% ETA {format_eta(eta)}")
                sys.stdout.flush()
        ...
        parts = [
            ...
            f"ce={avg_ce:.4f}",
            f"kl={avg_kl:.2f}",
            f"eval_exact={metrics['exact_acc']:.4f}",
            f"eval_pixel={metrics['pixel_acc']:.4f}",
        ...
        if epoch % 10 == 0:
            torch.save(
                ...
                save_dir / f"bottleneck_ep{epoch}.pt",
            )

    total_time = time.time() - global_start
    print(f"\nDone. Best eval exact accuracy: {best_acc:.4f}")
...
if __name__ == "__main__":
    ...
    p.add_argument("--save-dir", default="saves/bottleneck")
